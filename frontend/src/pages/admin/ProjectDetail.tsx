...
export default function ProjectDetail() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h2 className="text-xl font-semibold text-ink-900 dark:text-slate-100">{p.name}</h2>
      <p className="text-sm text-ink-400 mb-6">{p.projektnummer} · {p.status} · Fortschritt {p.fortschritt_pct}%</p>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-ink-100 dark:border-slate-700 p-4">
        <p className="text-sm font-medium text-ink-600 dark:text-slate-200 mb-3">Cluster in diesem Projekt</p>
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
          {data.cluster_liste.map((c: any) => (
            <button key={c.id} onClick={() => navigate(`/admin/cluster/${c.id}`)}
              ...
              <span className="text-sm text-ink-600 dark:text-slate-200">{c.name}</span>
              <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600">{c.status}</span>
            </button>
          ))}
          {data.cluster_liste.length === 0 && <p className="text-sm text-ink-400 py-2">Noch keine Cluster zugewiesen.</p>}
        </div>
      </div>
    </div>
  );
