...
const STATUS_FARBE: Record<string, string> = {
  bau: "bg-blue-100 text-blue-700", betrieb: "bg-signal-100 text-signal-700",
  abgeschlossen: "bg-slate-200 text-slate-600", storniert: "bg-conduit-100 text-conduit-700",
  pausiert: "bg-conduit-100 text-conduit-700",
};
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold text-ink-900 dark:text-slate-100">Projekte</h2>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-ink-100 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700">
        {projects.map((p) => (
          <button key={p.id} onClick={() => navigate(p.id)}
            ...
            <div>
              <p className="text-sm font-medium text-ink-900 dark:text-slate-100">{p.name} {p.projektnummer && <span className="text-ink-400 font-normal">({p.projektnummer})</span>}</p>
